...
def test_operator_dashboard_site_builds_index_and_ticker_pages_with_chart():
    ...
    assert "data-lightweight-price-chart" in site["tickers/MSFT.html"]
    assert "https://unpkg.com/lightweight-charts" in site["tickers/MSFT.html"]
    assert "LightweightCharts.createChart" in site["tickers/MSFT.html"]
    assert "TradingView Lightweight Charts" in site["tickers/MSFT.html"]
    assert "chart-fallback" in site["tickers/MSFT.html"]
    assert "data-range=\"1M\"" in site["tickers/MSFT.html"]
    assert "chart-tooltip" in site["tickers/MSFT.html"]
    assert "initLightweightPriceCharts" in site["tickers/MSFT.html"]
